    ...
    internal class HYModbus : Modbus
    {
        ...
        public override void DataExtract(CommData frame, byte[] buffer)
        {
            ...
            if (cmd == 0x03)
            {
                ...
                if (this.IsAscii == false)
                {
                    ...
                    for (int i = 0; i < byteCountLen; i++)
                        dataLen += buffer[startIdx + headerBytes.Length + i] << (8 * (byteCountLen - 1 - i));
                }
                else
                {
                    //Ascii → Rtu Bytes
                    ...
                    for (int i = 0; i < dataLenBytes.Length; i++)
                    {
                        dataLenBytes[i] = Convert.ToByte(
                            Encoding.ASCII.GetString(
                                new byte[] {
                                    buffer[startIdx + headerBytes.Length + (i * 2)],
                                    buffer[startIdx + headerBytes.Length + (i * 2) + 1]
                                }
                            )
                        );
                    }
                ...
                frameLen = headerBytes.Length + byteCountLen + dataLen + base.ErrCodeLength;
            }
            else if (cmd == 0x06 || cmd == 0x10)
            {
                ...
                frameLen = headerBytes.Length + bodyLen + base.ErrCodeLength;
            }
            else if (cmd >= 0x80)
            {
