...
namespace DotNetFrame.Frm
{
    ...
    public partial class FrmCommTester : Form
    {
        ...
        private void BtnSend_Click(object sender, EventArgs e)
        {
            ...
            SendData(
                txt: this.txtWrite.Text,
                addError: this.chkAddErrChk.Checked,
                isRepeat: this.chkRewrite.Checked,
                repeatCount: Convert.ToInt32(this.numRewrite.Value),
                repeatInfi: this.chkRewriteInfi.Checked);
        ...
        private void SetProtocolType(ProtocolType protocol)
        {
        ...
        private void SendData(string txt, bool addError, bool isRepeat, int repeatCount, bool repeatInfi)
        {
            this._recycleData = null;
            this._port.WriteQueue.Clear();

            byte[] bytes = ConvertTextToByte(txt);
            if (bytes == null) return;

            //전송 Data 등록
            this._recycleData = bytes;
            if (addError)
            {
                //에러코드 만들기
                byte[] errCode = this._port.Protocol.CreateErrCode(this._recycleData);
                this._recycleData = QYUtils.Comm.BytesAppend(this._recycleData, errCode);
            }
        ...
        private void BgWorker_DoWork(object sender, DoWorkEventArgs e)
        {
            while (true)
            {
                try
                {
                    ...
                    {
                        if (this._port.WriteQueue.Count == 0)
                        {
